...
module.exports = {
    ...
    setAndGetCrashReports: function(test) {
        ...
        lmHelpers.createLocMapUser(test, testUserEmail, 'dev1', function(auth, reply) {
            ...
            lmHelpers.api.post(test, '/v1/crashReport/' + reply.id, authWithData, function(res) {
                var now = new Date();
    ...
    resetUserAccount: function(test) {
        ...
        lmHelpers.createLocMapUser(test, email, 'dev1', function(auth, reply) {
            var userId = reply.id;
             lmHelpers.api.post(test, '/v1/admin/358405297258/accountRecovery', {data: {email: email}}, function(res) {
                 lmHelpers.createLocMapUser(test, email, 'differentdev', function(auth, reply) {
    ...
    resetNotAllowedOnNormalAccount: function(test) {
        ...
        lmHelpers.createLocMapUser(test, email, 'dev1', function(auth, reply) {
            var userId = reply.id;
            lmHelpers.api.post(test, '/v1/admin/358405297258/accountRecovery', {data: {email: email}}, {status: 401}, function(res) {
                lmHelpers.api.post(test, '/v1/signup', {data: {email: email, 'device_id': 'differentid'}}, {status: 401}, function(res) {
                    test.done();
                });
            });
        });
        ...
        lmHelpers.createLocMapUser(test, testUserEmail, 'dev1', function(auth, reply) {
            lmHelpers.createLocMapUser(test, testUserEmail2, 'dev2', function(auth2, reply2) {
                ...
                lmHelpers.api.post(test, '/v1/user/' + reply.id + '/location', auth, function(res) {
                    ...
                    lmHelpers.api.post(test, '/v1/user/' + reply.id + '/allow', auth, function(res) {
                        // User 2 has 1 contact, user 3. User 3 has not logged in yet.
                        auth2.data = {emails: [testUserEmail3]};
                        lmHelpers.api.post(test, '/v1/user/' + reply2.id + '/allow', auth2, function(res) {
                            // User 1 is using iOS.
                            auth.data = {apnToken: 'token'};
                            lmHelpers.api.post(test, '/v1/user/' + reply.id + '/apnToken', auth, function(res) {
                                // User 2 is using Android.
                                auth2.data = {gcmToken: 'token'};
                                lmHelpers.api.post(test, '/v1/user/' + reply2.id + '/gcmToken', auth2, function(res) {
                                    // User 1 is not visible.
                                    auth.data = {visibility: false};
                                    lmHelpers.api.put(test, '/v1/user/' + reply.id + '/visibility', auth, function(result) {
                                        // User 1 has accessed dashboard today.
                                        lmHelpers.api.get(test, '/v1/user/' + reply.id + '/dashboard', auth, function(result) {
