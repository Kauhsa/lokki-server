        ...
        for (var l in languages) {
            if (languages.hasOwnProperty(l)) {
                ...
                try {
                    ...
                    strings[lang] = JSON.parse(data);
                } catch (err) {
                    console.log('Failed to load or parse localization file: ' + file + '. Error: ' + err.message);
                    // to show real error in debugger log
                    //var data = fs.readFileSync(file, 'utf8');
                    //strings[lang] = JSON.parse(data);
                }
            }
    ...
    var formatString = function(text, fieldName1, fieldValue1, fieldName2, fieldValue2, fieldName3, fieldValue3) {
        ...
        if (fieldName1 !== undefined && fieldValue1 !== undefined) {
            ...
            if (textBeforeReplace === text) {
                console.log('LOC: string [' + text + '] does not have field %' + fieldName1 + '% to replace to ' + fieldValue1);
            }
        }
        ...
        if (fieldName2 !== undefined && fieldValue2 !== undefined) {
            ...
            if (textBeforeReplace === text) {
                console.log('LOC: string [' + text + '] does not have field %' + fieldName2 + '% to replace to ' + fieldValue2);
            }
        }
        ...
        if (fieldName3 !== undefined && fieldValue3 !== undefined) {
            ...
            if (textBeforeReplace === text) {
                console.log('LOC: string [' + text + '] does not have field %' + fieldName3 + '% to replace to ' + fieldValue3);
            }
        }
        return text;
    };
    ...
    this.getLocalizedString = function(lang, key, fieldName1, fieldValue1, fieldName2, fieldValue2, fieldName3, fieldValue3) {
        if (!strings.hasOwnProperty(lang)) {
            if (typeof lang === 'string' && lang.length === 2) {
                ...
                if (!foundLang) {
                    console.log('LOC: unknown language: ' + lang + ' requested string: ' + key);
        ...
        if (!strings[lang].hasOwnProperty(key)) {
            ...
            if (!strings[_getDefaultLanguage()].hasOwnProperty(key)) {
                console.log('LOC: unknown string requested: ' + key);
                return '';
            }
