...
var apnsErrorCallback = function(err, notification) {
    if (err !== 8) {
        console.log('ERROR: AppleNotificationService not delivered: ' + err + ' : ' + JSON.stringify(notification));
    }
};

var apnsErrorCallbackDev = function(err, notification) {
    if (err !== 8) {
        console.log('ERROR: AppleNotificationService dev not delivered: ' + err + ' : ' + JSON.stringify(notification));
    }
};

var apnsErrorCallbackAppStore = function(err, notification) {
    if (err !== 8) {
        console.log('ERROR: apnsErrorCallbackAppStore not delivered: ' + err + ' : ' + JSON.stringify(notification));
    }
};
...
AppleNotificationService.prototype.pushNotification = function(deviceToken, notificationText, payload, silent) {
    //console.log("Sending APN to " + deviceToken + ". text: " + notificationText + " payload: " + payload + " silent: " + silent);
